...
class LocalSpatialEncoding(nn.Module):
    def __init__(self, d, num_neighbors, device):
        ...
        self.mlp = SharedMLP(10, d, bn=False, activation_fn=nn.Sigmoid())
...
class AttentivePooling(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.mlp = SharedMLP(in_channels, out_channels, bn=False, activation_fn=nn.Sigmoid())
...
class LocalFeatureAggregation(nn.Module):
    def __init__(self, d_in, d_out, num_neighbors, device):
        ...
        self.mlp1 = SharedMLP(d_in, d_out//2, activation_fn=nn.Sigmoid())
        self.mlp2 = SharedMLP(d_out, 2*d_out)
        self.shortcut = SharedMLP(d_in, 2*d_out, bn=False)
...
class RandLANet(nn.Module):
    def __init__(self, d_in, num_classes, num_neighbors=16, decimation=4, device=torch.device('cpu')):
        ...
        self.mlp = SharedMLP(512, 512, activation_fn=nn.Sigmoid())
        ...
        self.fc_end = nn.Sequential(
            SharedMLP(8, 64, bn=False, activation_fn=nn.Sigmoid()),
            SharedMLP(64, 32, bn=False, activation_fn=nn.Sigmoid()),
            nn.Dropout(),
            SharedMLP(32, 32)
        )
        self.device = device
